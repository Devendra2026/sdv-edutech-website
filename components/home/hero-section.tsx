...
export default function HeroSection() {
  return (
    <section className="relative overflow-hidden border-b border-border/60 bg-background">
      {/* Background grid + soft tint (works with site-wide grid) */}
      ...
      <div className="relative max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-14 md:py-20 lg:py-24">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          <div className="space-y-6 md:space-y-7 order-2 lg:order-1">
            ...
            <div className="space-y-4">
              ...
              <p className="text-base md:text-lg text-muted-foreground leading-relaxed max-w-xl text-justify">
                Sdv Edutech Private Limited delivers next-generation digital solutions spanning enterprise software,
                cloud integration, IT consultancy, survey management, and skill development programs.
                We help government and corporate clients strengthen decision-making and build resilient
                digital ecosystems for long-term growth.
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-1">
              <Link
                href="/contact"
                className="inline-flex items-center justify-center gap-2 px-7 py-3.5 bg-primary text-primary-foreground rounded-lg font-semibold text-sm hover:bg-primary/90 transition-colors shadow-md shadow-primary/20"
              >
                Get in touch
              ...
              <Link
                href="/services"
                className="inline-flex items-center justify-center px-7 py-3.5 rounded-lg font-semibold text-sm border-2 border-primary text-primary hover:bg-primary/5 transition-colors"
              >
                Our services
              </Link>
            </div>
          ...
          {/* Hero image — fixed frame so Next/Image fills correctly */}
                ...
                <div className="absolute bottom-0 left-0 right-0 z-10 grid grid-cols-3 divide-x divide-white/20 bg-primary/95">
                  {stats.map((stat) => (
                    <div key={stat.label} className="px-2 sm:px-3 py-3.5 sm:py-4 text-center">
                      <p className="font-display text-lg sm:text-xl md:text-2xl font-bold text-white">
                        {stat.value}
                      </p>
                      <p className="text-[10px] sm:text-xs text-white/90 leading-tight mt-0.5">
                        {stat.label}
                      </p>
                    </div>
                  ))}
