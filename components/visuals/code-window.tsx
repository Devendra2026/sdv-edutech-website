...
export default function CodeWindowVisual({ className = '' }: { className?: string }) {
  const lines: { indent: number; tokens: { w: number; c: string }[] }[] = [
    ...
    },
    {
      indent: 1,
      tokens: [
        { w: 30, c: 'text-foreground/70' },
        { w: 70, c: 'text-primary' },
        { w: 24, c: 'text-foreground/40' },
      ],
    },
    {
      indent: 1,
      tokens: [
        { w: 48, c: 'text-foreground/70' },
        { w: 40, c: 'text-secondary' },
      ],
    },
    {
      indent: 2,
      tokens: [
        { w: 60, c: 'text-primary' },
        { w: 34, c: 'text-foreground/40' },
      ],
    },
    {
      indent: 2,
      tokens: [
        { w: 26, c: 'text-foreground/70' },
        { w: 80, c: 'text-primary' },
      ],
    },
    { indent: 1, tokens: [{ w: 18, c: 'text-foreground/40' }] },
    {
      indent: 0,
      tokens: [
        { w: 44, c: 'text-secondary' },
        { w: 64, c: 'text-primary' },
        { w: 28, c: 'text-foreground/40' },
      ],
    },
  ]
      ...
      >
        ...
        <div className='grid grid-cols-[88px_1fr] gap-3'>
          ...
          <div className='rounded-xl bg-card/70 border border-border p-3.5 space-y-2.5'>
            {lines.map((line, i) => (
              <div
                key={`${line.indent}-${line.tokens.map(t => `${t.w}-${t.c}`).join('-')}`}
