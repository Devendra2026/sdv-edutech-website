...
export default function ContactPage() {
  ...
  const contactInfo = [
    ...
    { icon: <Mail className="w-5 h-5" />, label: 'Email', value: 'info@sdvedutech.com', href: 'mailto:info@sdvedutech.com' },
    { icon: <MapPin className="w-5 h-5" />, label: 'Office', value: 'New Delhi, India', href: '#' },
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
          ...
          >
            {contactInfo.map((info, idx) => (
              ...
              >
                <div className="inline-flex p-3 rounded-xl icon-tile mb-4 group-hover:scale-105 transition-transform">{info.icon}</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  ...
                  <textarea name="message" value={formData.message} onChange={handleChange} required rows={5} className={`${inputClass} resize-none`} placeholder="Tell us about your project or inquiry..." />
                </div>
                <motion.button
                  type="submit"
            ...
            >
              <GisMapVisual className="flex-1" />
              <div className="glass rounded-2xl p-6 flex items-center gap-4">
                <div className="icon-tile p-3 w-12 h-12 flex-shrink-0">
                  <MapPin className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="font-display font-semibold text-foreground">Visit Us</h3>
                  <p className="text-muted-foreground text-sm">New Delhi, India — available for virtual and in-person meetings.</p>
                </div>
              </div>
            </motion.div>
          </div>
